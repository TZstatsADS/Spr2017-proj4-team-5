...
# train_x<-df.train[,-c(1,2)]
# train_y<-df.train[,2]
# test_x<-df.test[,-c(1,2)]
# test_y<-df.test[,2]
...
condition<-function(id){
  ...
  P_Ak_X<-NULL
  significant<-length(P.Ak.S.Co.X[[i]][P.Ak.S.Co.X[[i]]>0])
  for(k in seq(significant)){
        P_Ak_X[k]<-P.Co.X[i] * ( P.S.Co.X[i] * P.Ak.S.Co.X[[i]][P.Ak.S.Co.X[[i]]>0][k] + P.Ak.U.Co.X[i] * P.U.Co.X[i])
  }
  return(prod(P_Ak_X))
}

target<-NULL
for(i in seq(num.id)){ target[i]<-condition(i) }
which.max(target)
