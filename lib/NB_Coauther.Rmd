...
get.prob<-function(j){
  ...
  # flag <- ifelse(data$id.num.co==0,1,0)
...
################################
  find.identical<-function(i){
   # i=38
    ...
    if( sum(index) == 0){
        p_s_1<-sum(index)/dim(data.with.co)[1] #Seen
        return( list( p_s_1 = p_s_1,
                     p_s_0 = 1-p_s_1))
        }else{p_s_0<-sum(index)/dim(data.with.co)[1]  # Unseen
              return( list( p_s_0 = p_s_0,
                            p_s_1 = 1-p_s_0))
              }
     }
...
for(l in 1:num.with.co){
  prob.seen[l]<- list(find.identical(l))
}

# prob.seen 

colsum<-as.matrix(apply(data.with.co,2,sum))
total.num.co<-colsum[1,1]
